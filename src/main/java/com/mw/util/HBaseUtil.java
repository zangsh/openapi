...
public class HBaseUtil {
    ...
    public static void createTable(String tableName, String... FamilyColumn) {
        TableName tn = TableName.valueOf(tableName.toUpperCase());
        try {
            Admin admin = getCon().getAdmin();
            HTableDescriptor htd = new HTableDescriptor(tn);
            if (admin.tableExists(tn)) {
                logger.info(tn + " 表已经存在.");
            } else {
                for (String fc : FamilyColumn) {
                    HColumnDescriptor hcd = new HColumnDescriptor(fc.toUpperCase());
                    htd.addFamily(hcd);
                }
    ...
    // 创建表
    public static void createTable(String tableName, List<String> FamilyColumn) {
        TableName tn = TableName.valueOf(tableName.toUpperCase());
        try {
            Admin admin = getCon().getAdmin();
            ...
            if (admin.tableExists(tn)) {
                ...
                if (FamilyColumn.size() > 0) {
                    // 列族描述对象
                    Iterator<String> iterator = FamilyColumn.iterator();
                    while (iterator.hasNext()) {
                        HColumnDescriptor family = new HColumnDescriptor(iterator.next().toUpperCase());
                        hTableDescriptor.addFamily(family);
                    }
                } else {
                    HColumnDescriptor family = new HColumnDescriptor("0");
                    hTableDescriptor.addFamily(family);
                }
